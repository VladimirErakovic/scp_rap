CLASS zcl_generate_travel_data_ve DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_generate_travel_data_ve IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA: itab TYPE TABLE OF ztravel_ve.
...
*   delete existing entries in the database table
    DELETE FROM ztravel_ve.

*   insert the new table entries
    INSERT ztravel_ve FROM TABLE @itab.

*   check the result
    SELECT * FROM ztravel_ve INTO TABLE @itab.
    out->write( sy-dbcnt ).
